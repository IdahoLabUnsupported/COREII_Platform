...
"""
Ingest the unstructured CYOTE precursor analysis report files.

- Read the unstructured CYOTE text (docx, pdf) reports from a local directory.
- Create a DataReport instance for each report file, and link it to the parent DataSource for CYOTE.
...
from settings import RAW_DATA_DIRECTORY_CYOTE_REPORTS


def process_unstructured_reports_data():
    """
    Process and ingest the unstructured CYOTE analyst reports.
    """
    ...
    file_list = filesystem.list_directory_files(
        RAW_DATA_DIRECTORY_CYOTE_REPORTS, extensions=["docx", "pdf"]
    )
    ...
    for _filepath in file_list:
        ...
        new_report = Report(
            ...
            title=filename,
            report_type="CyOTE precursor analysis report",
            uri=filename,
...
def main():
    """Ingest the unstructured CYOTE analysis reports."""
    print("Processing unstructured CYOTE reports data...")
